...
== [ARTICLE] https://cacm.acm.org/magazines/2016/7/204032-why-google-stores-billions-of-lines-of-code-in-a-single-repository/fulltext[Why Google Stores Billions of Lines of Code in a Single Repository]

=== Lu le 2020-07-17, publié le 2016-07-?? par Rachel POTVIN, Josh LEVENBERG deux ingénieurs de Google.

* présentation du monorepo utilisé par google, des outils qui vont avec, et des avantages/inconvénients
* le début de l'article donne quelques chiffres, ils sont assez hallucinants, alors même qu'ils sont obsolètes car anciens !
** ~80 TiB de data
** 2 milliards de lignes de code, 9 millions de fichiers source
** toutes les semaines, 15 millions de lignes de code sont modifiées (soit l'équivalent du repo complet du noyau linux) sur 250k fichiers
** 25000 devs google, qui font 16k commits chaque jour (plus 24k commits faits par des outils automatiques)
** 500k queries / seconde en moyenne sur le repo (et jusqu'à 800k en pic)
* extraits (non-exhaustifs) :
** In sum, Google has developed a number of practices and tools to support its enormous monolithic codebase, including trunk-based development, the distributed source-code repository Piper, the workspace client CitC, and workflow-support-tools Critique, CodeSearch, Tricorder, and Rosie.
...
** Old APIs can be removed with confidence, because it can be proven that all callers have been migrated to new APIs.
** However, as the scale increases, code discovery can become more difficult, as standard tools like grep bog down.
** Some would argue this model, which relies on the extreme scalability of the Google build system, makes it too easy to add dependencies and reduces the incentive for software developers to produce stable and well-thought-out APIs.
** The fact that most Google code is available to all Google developers has led to a culture where some teams expect other developers to read their code rather than providing them with separate user documentation. There are pros and cons to this approach. No effort goes toward writing or keeping documentation up to date, but developers sometimes read more than the API code and end up relying on underlying implementation details.
