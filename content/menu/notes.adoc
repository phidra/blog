...
date: 2020-02-18T12:38:15+01:00
draft: true
---
...
Je croise souvent des articles/vidéos/posts/etc. dont j'ai envie de garder quelque chose. Jusqu'ici, ma façon canonique de le faire était d'en "extraire" la connaissance. Par exemple, si je regarde une <<video-sur-P-egal-NP,intéressante vidéo sur les problèmes P-difficiles et NP-difficiles>>, je vais ajouter ce que j'en retiens à ma base de notes sur le sujet, c'est à dire au fichier `notes/complexity.otl`, éventuellement en ajoutant le lien vers la vidéo en référence. Je prends ces notes (et d'autres) en utilisant https://www.vim.org/scripts/script.php?script_id=3515[vim-outliner] : en plus de les visualiser efficacement, le format est text-only, donc efficace, et facile à grepper.

Je veux tester sur cette page une autre façon de prendre des notes, probablement complémentaire du process que je suivais jusqu'ici : j'empile en vrac les liens vers les références intéressantes, assorties de quelques notes assez brutes. Cette pile n'a pas vocation à être structurée, elle est simplement triée par ordre de lecture.

=== Ce que j'en attends

* déjà, ça me permet de les partager plus simplement que l'arborescence de répertoires qu'est devenu mon projet `notes`.
* le côté déstructuré, associé au fait que les notes peuvent être minimales (quelques point à retenir), voire inexistantes (la simple présence du lien étant le signe que j'ai eu envie de pérenniser l'article) rendront mon process certes moins poussé, mais plus simple, puisque je n'ai pas besoin d'extraire et de structurer la connaissance pour la pérenniser.
...
*Attention* : ces notes sont prises avec *ma* vision des choses. Notamment, au sein d'un article, je ne prends pas de notes sur ce pour quoi j'estime ne pas avoir besoin de rappel/notes, ou sur ce qui n'éveille pas mon intérêt. Du coup, ne prenez pas ces notes comme des TL;DR des articles référencés.

Lorsque/si la page devient inutilisable du fait de sa taille, j'envisagerai d'autres pistes (pagination ? https://sebsauvage.net/wiki/doku.php?id=php:shaarli[shaarli] ?).
