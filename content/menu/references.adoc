...
* https://jvns.ca[le blog de Julia EVANS], j'aime beaucoup sa relation à l'apprentissage et l'auto-amélioration. Ses comics sont accessibles https://wizardzines.com/comics/[ici].
...
* https://developers.google.com/tech-writing/overview[Technical writing course] = cours dispensé par Google sur la rédaction de doc technique
* https://www.freebsd.org/cgi/man.cgi?query=sysexits[preferable exit codes for programs] = les conventions de FreeBSD pour les exit-codes, qui sont un peu l'équivalent des HTTP status codes.
