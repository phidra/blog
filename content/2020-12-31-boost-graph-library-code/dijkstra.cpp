...
int main(int, char*[])
{
    ...
    auto nameof = [&mygraph](VertexDescriptor const & vd) {
        return mygraph[vd].name;
    };

    auto display_vertex = [&mygraph](VertexDescriptor const & vd) {
        cout << "vertex " << mygraph[vd].name << " has coordinates (" << mygraph[vd].latitude << ";" << mygraph[vd].longitude << ")\n";
    };
    ...
    auto display_edge = [&mygraph, &nameof](EdgeDescriptor const & ed) {
        ...
        cout << "edge [" << nameof(from) << "-" << nameof(to) << "] is " << mygraph[ed].roadname << " with weight " << mygraph[ed].weight << "\n";
    };
    ...
    auto name_property_map = get(&VertexProperty::latitude, mygraph);
    cout << "latitude of D = " << get(name_property_map, D) << endl;
    cout << endl;
    ...
    cout << "Le plus court chemin de " << nameof(SOURCE) << " vers " << nameof(TARGET) << " a pour poids total : " << get(tentative_distances_pmap, TARGET) << endl;
