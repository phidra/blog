...
Voyons toutes ces belles notions mises en pratique. On va utiliser l'implémentation du R-tree par https://www.boost.org/doc/libs/1_72_0/libs/geometry/doc/html/geometry/spatial_indexes/rtree_quickstart.html[boost geometry].

Le code est en détails (pour ne pas surcharger), et également ici (TODO, donner le lien), avec les instructions pour le compiler.

* TODO : ici, donner le main complet.

Voici comment on insère un point (assorti de son id de node OSM) dans un R-tree :

.insérer un node dans un R-tree
[source,cpp]
----
...
À partir d'un extract OSM des rues de Bordeaux, on peut utiliser https://osmcode.org/libosmium/[libosmium] pour le parser, et construire le R-tree à requêter. Vous trouverez le code complet ICI (TODO : donner le lien).
...
.Requête du node OSM le plus proche d'un point quelconque
[source,cpp]
----
...
.Affichage de la distance entre le node et le point requêté
[source,cpp]
----
...
* gérer le lien vers le code
* écluser les TODOs
* merger avec mon projet git du blog
