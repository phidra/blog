...
RTree build_rtree_from_osm_data(const char* const osmfile) {
    ...
    try {
        ...
        using Index =
            osmium::index::map::SparseMmapArray<osmium::unsigned_object_id_type, osmium::Location>;
...
std::tuple<Point, osmium::object_id_type, float> find_nearest(const RTree& rtree,
                                                              const Point& requested_point) {
    std::vector<NodePoint> result;
    auto nb_found = rtree.query(bgi::nearest(requested_point, 1), std::back_inserter(result));
    if (nb_found == 0) {
        std::cerr << "unable to find nearest value in rtree (currently containing " << rtree.size()
                  << " segments)" << '\n';
        std::exit(3);
    }

    auto found = result.front();
    auto distance = bg::distance(found.first, requested_point,
                                 bg::strategy::distance::haversine<double>(6'371'000));
    return std::make_tuple(found.first, found.second, static_cast<float>(distance));
}

void display(std::tuple<Point, osmium::object_id_type, float>& nearest,
             float MAX_ALLOWED_DISTANCE_IN_M) {
    const Point& found_node = std::get<0>(nearest);
    osmium::object_id_type node_id = std::get<1>(nearest);
    float distance = std::get<2>(nearest);

    std::cout << "Nearest point is " << std::setprecision(2) << distance
              << " meters far : " << std::setprecision(8);
    ...
    if (static_cast<float>(distance) > MAX_ALLOWED_DISTANCE_IN_M) {
        std::cerr << "ERROR : nearest node seems further than the max allowed distance = "
                  << MAX_ALLOWED_DISTANCE_IN_M << " meters.\n";
        std::exit(4);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Querying nearest node to point : " << requested_longitude << ";"
              << requested_latitude << std::endl;
