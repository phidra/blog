...
* L'état initial du stream est : aucun des _state flags_ n'est setté.
* premier passage par `while` :
** getline commence lit le stream jusqu'à rencontrer le premier `\n`
** Il copie la chaîne `token1` dans `line`, puis il renvoie le stream.
** La lecture a bien réussi → le flag `failbit` n'est pas setté.
** Le stream n'est pas épuisé → le flag `eofbit` n'est pas setté.
** Aucun flag n'étant setté, `stream.good()` renvoie `true`, on passe donc dans le corps de la boucle, et on ajoute `token1` aux `tokens`.
* second passage par `while` :
** getline lit la suite du stream, et rencontre le second `\n`
** tout se passe comme pour le premier tour de boucle.
** Notamment, `getline` ne sette pas encore `eofbit` : comme il s'est arrêté au `\n`, il ne sait pas encore s'il reste ou non des caractères après ce `\n`.
** on passe donc ici aussi dans le corps de la boucle et on ajoute `token2` aux `tokens`.
* troisième passage par `while` :
** getline lit la suite du stream, et cosntate qu'il ne reste plus de caractères à lire
** il sette donc `eofbit`, et retourne immédiatement
** de plus, comme il n'a pas placé de caractère dans `line`, il sette également `failbit`
...
Qu'est-ce qui change si on utilise le code idiomatique `while(getline(stream, line))` au lieu de `while(getline(stream, line).good())` ? *RIEN* : on a vu que le code idiomatique évaluait `!stream.fail()` au lieu de `stream.good()`. Or, pour chacun des 3 passages par `while` décrits ci-dessus, ces deux appels se comportent de façon identique.
...
** Aucun flag n'étant setté, `stream.good()` renvoie `true`, on passe donc dans le corps de la boucle, et on ajoute `token1` aux `tokens`.
* second passage par `while` :
** getline lit la suite du stream, rencontre les caractères `"token2"` (qu'il place dans `line`), puis rencontre la fin du stream.
** getline ayant atteint la fin du stream, il sette `eofbit`
** La lecture a bien réussi (on a placé `token2` dans `line`) → le flag `failbit` n'est pas setté.
** La fin du stream ayant été atteinte, → getline sette le flag `eofbit`.
